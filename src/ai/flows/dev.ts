...
const generateContentFlow = ai.defineFlow(
  ...
  async (input) => {
    ...
    for (let i = 0; i < input.numPoems; i++) {
        ...
        // Extraemos una o dos palabras clave del título para el imageHint
        const titleKeywords = poem.title.toLowerCase().replace(/[^a-z\s]/g, '').split(/\s+/).filter(Boolean);
        const imageHint = titleKeywords.slice(0, 2).join(' ');
        ...
        const newPoem = await addAdminPoem({
            ...
            imageUrl: generatedImageUri, // Pasamos la URL de datos Base64 o la URL normal
            imageHint: imageHint,
        });
