...
import { Loader2, FolderHeart, Send, Play, Pause, Mic, Copy, Share2, Image as ImageIcon, Trash2 } from "lucide-react";
import {
  AlertDialog,
  AlertDialogAction,
  AlertDialogCancel,
...
import { useLocalStorage, useBrowserAPIs } from "@/hooks/use-browser-apis";
...
export default function CollectionsPage() {
    ...
    const { isClient, speechSynthesis, pageOrigin, navigator } = useBrowserAPIs();
    ...
    const [favoriteIds, setFavoriteIds, favoritesLoaded] = useLocalStorage<string[]>(FAVORITES_KEY, []);
    const [submittedPoems, setSubmittedPoems, submissionsLoaded] = useLocalStorage<Poem[]>(SUBMISSIONS_KEY, []);

    React.useEffect(() => {
        if (!isClient || !favoritesLoaded || !submissionsLoaded) return;
        ...
        loadCollections();
    }, [isClient, toast, favoriteIds, submittedPoems, favoritesLoaded, submissionsLoaded]);
    ...
    const handleDeleteFavorite = (e: React.MouseEvent, poemId: string) => {
        e.stopPropagation();
        setFavoriteIds(favoriteIds.filter(id => id !== poemId));
    ...
    const handleDeleteSubmission = (e: React.MouseEvent, poemId: string) => {
        e.stopPropagation();
        setSubmittedPoems(submittedPoems.filter(p => p.id !== poemId));
    ...
    const handleChangeImageLocally = (e: React.ChangeEvent<HTMLInputElement>, poemId: string, collectionName: string) => {
      ...
      reader.onloadend = () => {
          ...
          if (collectionName === "Mis Envíos") {
              const updatedPoems = submittedPoems.map(p => 
                  p.id === poemId ? { ...p, image: base64String, imageUrl: undefined } : p
              );
              setSubmittedPoems(updatedPoems);
          }
    ...
    if (!isClient || !favoritesLoaded || !submissionsLoaded) {
        return (
            <MainLayout>
